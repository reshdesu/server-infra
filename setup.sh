...
if [ ! -f "$REPO_DIR/arr-stack/.env" ]; then
    ...
    if grep -q "^MEDIA_ROOT=" "$REPO_DIR/arr-stack/.env"; then
        sed -i "s|^MEDIA_ROOT=.*|MEDIA_ROOT=$USER_MEDIA_ROOT|" "$REPO_DIR/arr-stack/.env"
    else
        echo "MEDIA_ROOT=$USER_MEDIA_ROOT" >> "$REPO_DIR/arr-stack/.env"
    fi
